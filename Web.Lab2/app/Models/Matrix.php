...
class Matrix extends Model
{
    ...
    function __construct(int $size=null, array $data=null, $squareCheck=TRUE) {
        ...
        {
            if(empty($size)) {
                $this->rowsCount = $this->colsCount = rand(4,9);
            }
            else {
    ...
    // }
        ...
        public function getMirrorH()
        {
            ...
            for($i = 0; $i < floor($this->rowsCount/2); $i++)
                {
                    $result->data[$i] = $this->data[$this->rowsCount-$i-1];
                    $result->data[$this->rowsCount-$i-1] = $this->data[$i];
                }
            return $result;
        }
        ...
        public function getMirrorV()
        {
            ...
            for($i = 0; $i < $this->rowsCount; $i++)
                for($j = 0; $j < floor($this->colsCount/2); $j++)
                    {
                        $result->data[$i][$j] = $this->data[$i][$this->colsCount-$j - 1];
                        $result->data[$i][$this->colsCount-$j - 1] = $this->data[$i][$j];
                    }
            return $result;            
        }
        ...
        public function pairCols()
        {
            $result = new Matrix(null, $this->data);
            for($i = 0; $i < $this->rowsCount; $i++)
                for($j = 0; $j < $this->colsCount - $this->colsCount % 2; $j+=2)
                    {
